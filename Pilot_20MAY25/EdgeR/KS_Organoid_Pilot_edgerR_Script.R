...
# This is the script for differential expression for the pilot experiment from Konrad's mouse organoids

# Read in the raw data and metadata ----
raw.data <- read.delim("salmon.merged.gene_counts.tsv", check.names=FALSE, stringsAsFactors=FALSE,row.names="gene_id")
raw.data <- read.delim("salmon.merged.gene_counts.tsv", check.names=FALSE, stringsAsFactors=FALSE,row.names="gene_id")
# Metadata
metadata <- read.delim("lenvatinib_metadata.txt",check.names=FALSE, stringsAsFactors=FALSE)
metadata$Treatment <- factor(metadata$Treatment, levels=c("No","Yes")) # Simple 2v2 test

## Because it seems like the metadata is assigned based off of the order of the rows in the raw gene expression table, this is my workaround for now (not really feasible for larger datasets)
raw.data <- raw.data[c("gene_name","N.C","N.100","T.C","T.100")]

# Create edgeR list object (called DGEList)
y <- DGEList(counts=raw.data,group=metadata$Treatment)

# Filtering out lowly expressed genes and re-calculating library size after filtering
keep <- filterByExpr(y)
